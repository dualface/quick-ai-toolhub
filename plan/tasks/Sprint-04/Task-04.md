...
将 `review-aggregation-tool` 接入 `Task Orchestrator` 的 review 阶段，落实补充审查、证据选择和统一流程决策。
...
- 基于聚合结果决定 `return_to_developer` / `await_human` / `open_task_pr`
- 接入补充审查或二次验证分支
...
- 本任务关注“如何使用聚合结果”，不再重新实现一套 findings 聚合内核
